...
import { 
  Activity, Bell, Brain, Calendar, ChevronRight, FileText, Heart, Mail, 
  MessageCircle, Moon, Phone, Pill, Play, Plus, Sun, User, Sparkles, 
  TrendingUp, Target, Gamepad2, Clock, Award, BarChart2, X, CheckCircle, 
  RefreshCw, Apple, GraduationCap 
} from 'lucide-react'
...
interface NotificationsPanelProps {
  notifications: ChildData['notifications']
  onClose: () => void
}
...
const MentalHealthModal: React.FC<MentalHealthModalProps> = ({ child, onClose }) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
...
const NutritionModal: React.FC<NutritionModalProps> = ({ child, onClose }) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
...
interface EducationProgressModalProps {
  child: ChildData
  onClose: () => void
}

const EducationProgressModal: React.FC<EducationProgressModalProps> = ({ child, onClose }) => {
