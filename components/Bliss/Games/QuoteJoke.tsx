...
export default function QuoteJoke() {
    ...
    const fetchJoke = async () => {
        try {
            const res = await axios.get<JokeResponse>(
                "https://v2.jokeapi.dev/joke/Any?type=single"
            );
            setJoke(res.data.joke || "Couldn't fetch a joke this time.");
        } catch (err) {
    ...
    const fetchQuote = async () => {
        try {
            const res = await axios.get<QuoteResponse[]>("https://api.api-ninjas.com/v1/quotes", {
                headers: {
