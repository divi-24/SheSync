...
interface SymptomSeverityMapping {
    [key: string]: { value: number; color: string; bgColor?: string };
}
...
const symptomSeverityMapping: SymptomSeverityMapping = {
...
const MoodTrendChart = ({
    ...
    const moodValues = {
        Happy: { value: 100, color: "#10B981" },
        Energized: { value: 80, color: "#3B82F6" },
        Calm: { value: 60, color: "#8B5CF6" },
        Tired: { value: 40, color: "#F59E0B" },
        Sad: { value: 20, color: "#6B7280" },
        Angry: { value: 0, color: "#EF4444" },
    };
    ...
    const chartData = useMemo(() => {
        return moodTypes
            .map((mood) => ({
                ...mood,
                ...(moodValues[mood.type as keyof typeof moodValues] || {
                    value: 50,
                    color: "#6B7280",
                }),
            }))
...
const PCOSRiskChart = ({ pcosReport }: { pcosReport: string | null }) => {
    const [riskLevel, riskCategory] = useMemo(() => {
        if (!pcosReport) return [0, "Low"];
        try {
...
const AnalysisSummaryChart = ({ pcosReport }: { pcosReport: string | null }) => {
...
export default function Diagnosis() {
    ...
    const [pcosReport, setPcosReport] = useState<string | null>(null);
                    ...
                    <div className="bg-pink-50 dark:bg-gray-800 rounded-lg shadow-lg p-8 space-y-8">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
                                        {Object.entries(categorizedSymptoms).map(
                                            ([category, symptomsList]) => (
                                                <div
                                                    key={category}
                                                    className={`dark:bg-gray-700 bg-white rounded-lg shadow p-4 border-l-4 ${category === "Menstrual Symptoms"
                                                            ? "border-pink-500"
                            ...
                            <>
                                {showCharts && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        />
                                        {pcosReport && <AnalysisSummaryChart pcosReport={pcosReport} />}
                                    </div>
                                )}
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <button
                                ...
                                disabled={isLoading}
                                className={`bg-pink-500 hover:bg-pink-600 text-white font-medium py-3 px-8 rounded-md text-lg transition duration-300 shadow-md ${isLoading ? "opacity-50 cursor-not-allowed" : ""
                                    }`}
                            >
